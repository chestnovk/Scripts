...
def check_input_parameters(vm_name, vm_ip, vm_iso, vm_ks):
    ...
    # Difficult to decide. You may want no to set ip. 
    # Best choise - use the S.Zenkov /mnt/sin/addips and parse it somehow to set external address if needed
    ...
    # Check vm_ks. I assume that all ks.cfg files are located in the "default" ks server. So we could just touch it from the node.
...
def create_vm(vm_name, vm_ip, vm_iso, vm_ks):
    ...
    # Way #1 - Simple. Create VM by executing prlctl
    # Way #2 - Difficult. Create VM by uisng SDK.
